...
__global__ void warp_tiling_tensor_mat_mul_kernel(float *d_A_ptr, float *d_B_ptr, float *d_C_ptr, int M, int N, int K)
{
    int warpId  = threadIdx.y;
    int warpRow = warpId / WARPS_PER_BLOCK_N;
    int warpCol = warpId % WARPS_PER_BLOCK_N;

    // shared memory: 블록 전체가 담당하는 BLOCK_TILE_M x WMMA_K (A), WMMA_K x BLOCK_TILE_N (B)
    __shared__ float As[BLOCK_TILE_M][WMMA_K];
    __shared__ float Bs[WMMA_K][BLOCK_TILE_N];

    // ---- fragment 배열 선언 (이게 이번 버전의 핵심 변경점) ----
    ...
    // 이 워프가 shared memory 안에서 담당하는 영역의 시작 오프셋
    int warpRowOffset = warpRow * WARP_TILE_M; // As 안에서 내 워프 행 시작점
    int warpColOffset = warpCol * WARP_TILE_N; // Bs 안에서 내 워프 열 시작점

    for (int k0 = 0; k0 < K; k0 += WMMA_K) {

        // ---- global -> shared 로드 (블록 전체 협력) ----
        const int As_elems = BLOCK_TILE_M * WMMA_K;
        for (int idx = flatTid; idx < As_elems; idx += threadsPerBlock) {
            int r = idx / WMMA_K;
            int c = idx % WMMA_K;
            As[r][c] = d_A_ptr[(blockRow + r) * K + (k0 + c)];
        }
        ...
        for (int idx = flatTid; idx < Bs_elems; idx += threadsPerBlock) {
            ...
            Bs[r][c] = d_B_ptr[(k0 + r) * N + (blockCol + c)];
        }

        __syncthreads();

        // ---- shared -> fragment 로드 (워프당 TILES_PER_WARP_M / N개씩) ----
        for (int i = 0; i < TILES_PER_WARP_M; ++i) {
        ...
        // ---- 핵심: a_frag[i] 하나를 b_frag[j] 여러 개와 재사용 ----
        // load 횟수: TILES_PER_WARP_M + TILES_PER_WARP_N
        // compute 횟수: TILES_PER_WARP_M * TILES_PER_WARP_N (재사용 효과)
        for (int i = 0; i < TILES_PER_WARP_M; ++i) {
            for (int j = 0; j < TILES_PER_WARP_N; ++j) {
    ...
    // ---- 결과 저장: 워프가 담당하는 TILES_PER_WARP_M x TILES_PER_WARP_N개 타일을 각각 저장 ----
    for (int i = 0; i < TILES_PER_WARP_M; ++i) {
        for (int j = 0; j < TILES_PER_WARP_N; ++j) {
...
// 호스트 wrapper
// ============================================================================
void warp_tiling_tensor_mat_mul(float *d_A, float *d_B, float *d_C, int M, int N, int K)
{
    dim3 dim_block(THREADS_PER_WARP, WARPS_PER_BLOCK);
    dim3 dim_grid(M / BLOCK_TILE_M, N / BLOCK_TILE_N);

    warp_tiling_tensor_mat_mul_kernel<<<dim_grid, dim_block>>>(d_A, d_B, d_C, M, N, K);
...
int main(int argc, char **argv) {
    ...
    printf("[TF32 Warp Tiling] WARPS=%dx%d, TILES_PER_WARP=%dx%d, BLOCK_TILE=%dx%d\n",
